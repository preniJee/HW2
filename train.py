...
def train(hidden_units, train_data_path, l_rate, n_epoch, save_path,max_seq_length=None):

    f1 = Features(train_data_path, stopwords_file="stopwords.txt")
    # print(f1.avg_len, f1.stdv_len)
    if max_seq_length is None:
        ### setting the max sequence length based on the dataset features and experimetnal setup
        if "odia" or "questions" in train_data_path:
            max_seq_length=f1.max_len
        elif  "product" in  train_data_path:
            max_seq_length = int(f1.avg_len + f1.stdv_len)
        else :
            max_seq_length=60
        # print(max_seq_length)

    X_train= f1.get_features(preprocessed=f1.preprocessed_text, max_seq_length=max_seq_length)
    ...
    model = NNComp(feature_dim=n_inputs, h_units=hidden_units, output_dim=n_outputs)
...
if __name__ == '__main__':
    ...
    train(hidden_units=args.u,train_data_path=args.i, l_rate=args.l, n_epoch=args.e, save_path=args.o,max_seq_length=args.f)
